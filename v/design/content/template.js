  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        coverTmpl: "quotes/Square/${id}.png",
        items: [
          "Aim_for_the_moon",
...
function renderTemplates(templates) {
  ...
  templates.forEach(category => {
    html += `<section class="mb-4 hidden px-4">`;
    // 主标题
    html += `<h2 class="text-3xl font-bold mb-6 pl-3 border-l-4 border-purple-500 text-slate-800 dark:text-slate-100 hidden">${category.title}</h2>`;
    ...
    // 渲染所有 items
    // 分成两列
    html += `<div class="columns-2 gap-2">`;
    allItems.forEach(item => {
      html += `<a href="/v/design/?id=${item.link}" class="design-item block mb-2 relative overflow-hidden rounded-xl shadow-md hover:shadow-lg transition-all duration-300 break-inside-avoid min-h-20" target="_blank">`;
      // 图片容器，限制高度并垂直居中裁剪
      html += `<div class="flex items-center justify-center max-h-1/3 overflow-hidden">`;
      html += `<img data-src="https://xiaomingyan.com/static/v/design/templates/${item.cover}" src="https://xiaomingyan.com/static/common/d.gif" alt="${item.id}" 
              class="lazyload w-full h-full max-h-1/3  object-top object-cover transform transition-transform duration-300 hover:scale-105 
...
// 生成分类过滤器
const activeClz = "btn-primary";
const inactiveClz = "btn-secondary";
function renderFilter(types, titles) {
  render(null, ()=>{
    return types.map((type, index) => {
      ...
      return `<button class="filter-item ${activeClasses} btn-sm" data-id="${type}">${titles[index]}</button>`;
    });
  }, filterContainer);
}
...
function switchCategory(type, target) {
  ...
  // 高亮选中 activeClz
  const prevActive = filterContainer.querySelector(`.${activeClz}`);
...
function renderAll() {
  ...
  const refs = render(null,()=>{
    return `
    <div data-id="filterContainer" class="grid grid-cols-4 gap-2 p-4 mb-4 bg-white z-10 shadow-md"></div>
    <div data-id="posterContainer" class="flex-1 overflow-auto"></div>
    `;
  }, container);

  posterContainer = refs.posterContainer;
  filterContainer = refs.filterContainer;
  posterContainer.innerHTML = renderTemplates(completeTemplates);
  sections = posterContainer.querySelectorAll("section");
  // 生成分类过滤器
  ...
  delegator.on(posterContainer, "click", ".design-item", (event, target) => {
    // 阻止默认行为
    event.preventDefault();
    // 弹框让用户确认是否在本窗口打开
    const url = target.getAttribute("href");
    
    if (confirm("您确定要替换当前海报吗？")) {
      // location.href = url;
      // initCanvasFromTemplate
      emitter.emit('canvas:loadTemplate',url.split('id=')[1]);
