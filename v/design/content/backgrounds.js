...
function createFinalListView(items) {
        ...
        <div class="flex items-center gap-4">
          <div class="relative w-full flex-1">
            <input
              ...
              class="input w-full px-3 py-1.5 rounded-lg text-sm pr-8"
            />
  ...
  refs.search.addEventListener("keyup", (event) => {
    if (event.key === "Enter") {
      search();
    }
  });
  // blur
  refs.search.addEventListener('blur',search);
  ...
  loadViewData(title, type, level, prevType).then(async (data) => {
    ...
    if(level === 1){
      refs = render('', (d,e,f,_if) => {
        return [
          `<div class="grid grid-cols-3 py-4 px-4 gap-2">
            ...
            <button class="btn-primary bg-green-500 hover:bg-green-600" data-id="uploadBackgroundImage"><i class="vicon-upload text-lg mr-1"></i>图片</button>
          </div>
      ...
      delegator.on(contentDiv, 'click', '[data-id^="tab-"]', (e, target) => {
        ...
        if (contentId === 'unsplash') {
          emitter.emit("operation:imagePicker:show", {
            ...
            color: '',
            onSelect: (url) => {
              emitter.emit("operation:background:init", {
                type: 'url',
                data: url
              });
            }
...
function loadViewData(title, type, level, prevType) {
  ...
  return new Promise((resolve) => {
      ...
      if (type === "backgrounds") {
        loadScript("https://xiaomingyan.com/static/v/design/assets/backgrounds.en.js", () => {
          // ZMprops.backgrounds = [
          //   {
          //     name: "abstract",
          //     desc: "Abstract",
          ...
          data = ZMprops.backgrounds;
          //   遍历 data 转成可渲染的数据
          
          data = data.map((item) => {
            return {
              title: backgroundsNameMap[item.desc] || item.desc,
              type: item.name,
              ...
              items: item.imgs.slice(0, 10).map((img, index) => {
                return {
                  id: index + 1,
                  title: `${index + 1}`,
                  cover: `https://xiaomingyan.com/static/v/design/assets/backgrounds/${item.name}/${img.src}`,
                  dataType: 'background',
                  ...img
                };
              }),
      ...
      if(prevType === "backgrounds"){
        // 在 ZMprops.backgrounds 数组中找到 item.name === type 的对象
        let data;
        ZMprops.backgrounds.forEach((item) => {
          if (item.name === type) {
            data = item;
          }
        });
        ...
        data = data.imgs.map((img, index) => {
          return {
            ...
            title: `${index + 1}`,
            hasMore: false,
            cover: `https://xiaomingyan.com/static/v/design/assets/backgrounds/${type}/${img.src}`,
            dataType: 'ornament'
          };
        });
        resolve(data);
...
function renderContentCallback(containerEl, items) {
  ...
  render('',(d,e,f,_if) => {
    return f(items,(item) => {
      ...
      const thumb = item.cover.replace(/\/([^/]+)$/, '/thumb.$1');
      return `<div class="${className}">
        <img src="${thumb}" data-src="${thumb}" data-original="${item.cover}" data-id="background" alt="${title}" class="object-cover w-full h-full">
      </div>`;
    });
  }, containerEl);
}
...
// const data = [
//     { title: "装饰图案", type: "backgrounds",hasMore:true, items:[
//         {
//             id: 1,
//             title: "装饰图案1",
...
emitter.on("content:load", async (event) => {
  ...
  if (type === "backgrounds") {
    ...
    if(!isBind){
      ...
      container.innerHTML = "";
      const mainView = createView("背景", "backgrounds", 1, false, "");
      viewStack.length = 0;
      ...
      delegator.on(container, "click", "[data-id]", (event, target) => {
        ...
        // if (id === "customGradientColor") {
        //   emitter.emit("operation:background:init", {
        //     type: 'gradient'
        //   });
        // }
        if (id === "uploadBackgroundImage") {
          emitter.emit("operation:background:init", {
            type: 'upload'
          });
          // operation:upload-image:init
          // emitter.emit("operation:upload-image:init", 'background');
        }
        if (id === "removeButton") {
          emitter.emit("operation:background:init", {
            type: 'remove'
